...
var servents = {
	...
	'lancer': [18, 78, 87, 102, 134, 140, 141, 146, 181, 183, 193, 214, 217, 228, 252, 266, 279, 283, 288, 313, 347, 419, 428, 439, 449],
	...
	'avenger': [147, 158, 328, 356, 388, 420],
...
function init(state = 0){
	...
	resetButton.onclick = function(){
		if (confirm("確定要清空這個帳號的所有持有和標記資料嗎？此操作無法復原。")) {
	...
	resetMarkButton.onclick = function(){
		if (confirm("確定要清空這個帳號的所有標記嗎？")) {
			let data = getData(FGO_STORAGE);
			data.forEach(u => u.mark = 0);
			setData(FGO_STORAGE, data.filter(u => u.npLv > 0));
			location.reload();
		}
	}
